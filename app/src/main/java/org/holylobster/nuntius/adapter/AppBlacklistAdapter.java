...
public class AppBlacklistAdapter extends RecyclerView.Adapter<AppBlacklistAdapter.ViewHolder> {
    ...
    public AppBlacklistAdapter(List<ApplicationInfo> packages, PackageManager pm) {
        this.pm= pm;
        this.appInfos = new ArrayList<>(packages);
    }

    public AppBlacklistAdapter(ArrayList<String> packages, PackageManager pm) {
        this.pm= pm;
        appInfos = new ArrayList<>();
        for (int i = 0; i< packages.size(); i++){
            try {
                appInfos.add(pm.getApplicationInfo(packages.get(i), 0));
    ...
    public void refresh(ArrayList<String> packages){
        appInfos = new ArrayList<>();
        for (int i = 0; i< packages.size(); i++){
            try {
                appInfos.add(pm.getApplicationInfo(packages.get(i), 0));
